...
export const createSociety = async (society: Omit<Society, 'id' | 'createdAt' | 'updatedAt'>): Promise<Society | null> => {
  // TODO: Replace with actual API call
  try {
    const response = await fetch('/api/societies', {
      method: 'POST',
...
export const updateSociety = async (id: string, society: Partial<Society>): Promise<Society | null> => {
  // TODO: Replace with actual API call
  try {
    const response = await fetch(`/api/societies/${id}`, {
      method: 'PUT',
