...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Unvalidated `response.json()` + `as` assertions** on the client (and some loose `await response.json()` locals) — not the same as `any`, but no runtime check that payloads match the asserted shape. | Low      | Prefer Zod (or shared types + `safeParse`) where mismatches would be painful; dashboard [`client/src/pages/dashboard/api.ts`](../client/src/pages/dashboard/api.ts) and [`UsersPage`](../client/src/pages/UsersPage.tsx) are typical call sites. **Update 2026-04:** a repo-wide search found **no** explicit TypeScript `any` (`: any`, `as any`) in [`server/routes/`](../server/routes/) or [`client/src/`](../client/src/); older review examples (`debts` `updateData`, backoffice JWT, Users page) are now concretely typed. |
...
CI is documented in [docs/DEVELOPMENT.md](DEVELOPMENT.md) (workflows: [`.github/workflows/`](../.github/workflows/) — `ci.yml`, `e2e.yml`, `security-audit.yml`).
