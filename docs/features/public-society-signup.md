...
- Form collects society profile (name, optional short description max 250 chars, acronym, contact), subdomain when multitenancy is enabled (`TENANT_APEX_DOMAIN` on the API; the UI reads `apexDomain` from `GET /api/public/tenant-by-host`, with `VITE_TENANT_APEX_DOMAIN` as an optional client hint while that request is loading), administrator name/email/password, **marketing opt-in** (off by default), and **terms acceptance** (required).
...
- Email contains a link to `/egiaztatu-posta?token=…`. Base URL: `APP_PUBLIC_ORIGIN` if set; else in `NODE_ENV=production` and with `TENANT_APEX_DOMAIN`, `https://{TENANT_APEX_DOMAIN}`; else `http://localhost:${PORT}` (dev).
