...
export function NotesManagementPage() {
  ...
  const [confirmPushDialog, setConfirmPushDialog] = useState<{
    ...
    noteId: string;
    noteTitle: string;
  }>({ open: false, noteId: "", noteTitle: "" });
  const [confirmRevertDialog, setConfirmRevertDialog] = useState<{
    open: boolean;
    noteId: string;
    noteTitle: string;
  }>({ open: false, noteId: "", noteTitle: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasValidContent = formData.messages.every(msg => msg.title.trim() && msg.content.trim());
  ...
  const handleDelete = async (noteId: string) => {
    if (
      !confirm("Ziur zaude ohar hau ezabatu nahi duzula? / ¿Estás seguro de eliminar esta nota?")
    ) {
      return;
    }
  ...
  const handlePushNotification = async (noteId: string) => {
    try {
      ...
      setNotes(prev =>
        prev.map(note => (note.id === noteId ? { ...note, notifyUsers: true } : note))
      );
  ...
  const handleRevertNotification = async (noteId: string) => {
    try {
      ...
      // Update the note in the local state
      setNotes(prev =>
        prev.map(note => (note.id === noteId ? { ...note, notifyUsers: false } : note))
      );
  ...
  return (
    <ErrorBoundary FallbackComponent={ErrorFallback}>
              ...
              <Card key={note.id}>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <p className="text-sm">{getNoteDisplayContent(note).content}</p>
                  </div>
                </CardContent>
              </Card>
            ))
      ...
      <AlertDialog
        open={confirmPushDialog.open}
        onOpenChange={open =>
          !open && setConfirmPushDialog({ open: false, noteId: "", noteTitle: "" })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setConfirmPushDialog({ open: false, noteId: "", noteTitle: "" })}
            >
              {t("cancel")}
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog
        ...
        onOpenChange={open =>
          !open && setConfirmRevertDialog({ open: false, noteId: "", noteTitle: "" })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setConfirmRevertDialog({ open: false, noteId: "", noteTitle: "" })}
            >
              {t("cancel")}
            </AlertDialogCancel>
            <AlertDialogAction
