...
When('I increase the quantity of {string} to {int}', async function (productName, quantity) {
  const page = getPage();
...
When('I should see {string} in the items table with quantity {int}', async function (productName, quantity) {
  const page = getPage();
  ...
  assert.equal(parseInt(quantityText), quantity, `Quantity should be ${quantity}`);
});
...
Then('the cart should still contain {string}', async function (productName) {
  const page = getPage();
  const cartItem = page.locator(`[data-testid="cart-item"]:has-text("${productName}")`);
...
Then('the consumption should appear in the consumption list', async function () {
  ...
  assert.ok(consumptionId.length > 0, 'Consumption should have an ID');
