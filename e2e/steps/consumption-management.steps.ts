...
When(
  ...
  async function (productName: string, quantity: number) {
    const page = getPage();
    if (!page) throw new Error("Page not available");
    ...
    for (let i = 1; i < quantity; i++) {
      await cartItem
        .locator('[data-testid^="button-increase-quantity-"]:not([data-testid^="mobile-"])')
        .click();
...
When(
  "I should see {string} in the items table with quantity {int}",
  async function (productName: string, quantity: number) {
    const page = getPage();
    if (!page) throw new Error("Page not available");
    const itemRow = page.locator(
      `[data-testid="confirmation-items-table"] tr:has-text("${productName}")`
    );
...
Then("I should see a success message", async function () {
  ...
  const successToast = page.locator(
    '[data-testid^="toast-"]:has-text("itxita"), [data-testid^="toast-"]:has-text("cerrada")'
  );
  await successToast.waitFor({ state: "visible", timeout: 5000 });

  const successMessage = await successToast.textContent();
  assert.ok(
    successMessage?.includes("itxita") || successMessage?.includes("cerrada"),
...
Then("the cart should be empty", async function () {
  ...
  const cartItems = await page
    .locator('[data-testid^="cart-item-"]:not([data-testid^="mobile-"])')
    .count();
...
Then("the cart should still contain {string}", async function (productName: string) {
  ...
  const cartItem = page.locator(
    `[data-testid^="cart-item-"]:has-text("${productName}"):not([data-testid^="mobile-"])`
  );
...
Then("the consumption should appear in the consumption list", async function () {
  ...
  assert.ok(
    consumptionText?.includes("Itxita") || consumptionText?.includes("closed"),
    "Consumption should have closed status"
  );

  // Check that it has a valid consumption ID format
  const consumptionId = await firstConsumptionRow
    .locator('[data-testid="consumption-id"]')
    .textContent();
  assert.ok((consumptionId?.length || 0) > 0, "Consumption should have an ID");

  // Verify the total amount is displayed correctly
  const totalAmount = await firstConsumptionRow
    .locator('[data-testid="consumption-total"]')
    .textContent();
