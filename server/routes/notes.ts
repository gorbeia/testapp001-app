...
import { db } from "../db";
import { notes, noteMessages, users, notifications, notificationMessages, type User } from "@shared/schema";
...
async function convertNoteToNotifications(noteId: string, societyId: string) {
  try {
    ...
    const messages = noteWithMessages
...
async function removeNoteNotifications(noteId: string, societyId: string) {
  try {
    ...
    const deletedNotifications = await db
      ...
      .where(and(
        eq(notifications.referenceId, noteId),
...
export function registerNoteRoutes(app: Express) {
  ...
  app.put("/api/notes/:id/notify", sessionMiddleware, requireAuth, requireAdmin, async (req, res, next) => {
    try {
      const { id } = req.params;
